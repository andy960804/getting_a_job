...
## 버전 게이트 흐름
```
v1 초안 → [비즈니스 레터 9항] → v2 리라이팅 → [Anti-AI 16항] → v3 최종 → [제출 전 점검 7항] → 제출
```
...
- **[2026.04.21]** v1 초안부터 반드시 ~습니다체 적용. ~했다/~이다 종결어미는 v1에서도 금지. 링커리어 280건 전수 분석 결과 예외 없이 100% 습니다체 확인됨.
- **[2026.04.21]** 단락 연결 패턴 필수: 배경(1~2줄) → "이에/우선" + 행동 동사 → "그 결과" + 수치. "이에 저는" 패턴 금지 — "저는" 삭제.
- **[2026.04.21]** 소제목은 [대괄호] 형식이 표준. 소제목 없이 문항 작성 금지.

## Hard Constraints
- `[ GATE 0 ]` 모든 문장은 ~습니다/~했습니다/~겠습니다 체로 종결. ~했다/~이다/~봤다 단 1개도 금지. 소제목은 [대괄호] 형식 필수. 소제목은 반드시 [HOW + RESULT] 형식으로 작성.
- `[ GATE 1 ]` 경험 카드 + analyst 전략 메모 없이 집필 시작 금지
- `[ GATE 2 ]` 비즈니스 레터 9항 통과 없이 v2 진입 불가
- `[ GATE 3 ]` Anti-AI 16항 통과 없이 v3 생성 불가
